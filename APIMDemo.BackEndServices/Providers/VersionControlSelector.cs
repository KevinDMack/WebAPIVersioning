...
namespace APIMDemo.BackEndServices
{
    ...
    {
        public const string UrlEmbedded = "UrlEmbedded";
        public const string DefaultVersionSelector = "DefaultVersionSelector";
    ...
    public class VersionControllerSelector : DefaultHttpControllerSelector
    {
        ...
        public override HttpControllerDescriptor SelectController(HttpRequestMessage request)
        {
            ...
            if (controllers.TryGetValue(controllerName, out controllerDescriptor))
            {
                ...
                switch (ConfigurationManager.AppSettings["VersioningSchema"])
                {
                    case VersioningSchema.QueryString:
                        version = GetVersionFromQueryString(request);
                        break;
                    case VersioningSchema.Header:
                        version = GetVersionFromHeader(request);
                        break;
                    default:
                        version = "2";
                        break;
        ...
        {
            const string HEADER_NAME = "X-API-Version";
